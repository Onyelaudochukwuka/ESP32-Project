...
<path d='M7 3.33782C8.47087 2.48697 10.1786 2 12 2C17.5228 2 22 6.47715 22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 10.1786 2.48697 8.47087 3.33782 7' stroke='#000000' stroke-width='1.5' stroke-linecap='round'/>
    ...
    return '#' + r + g + b;
      ...
      function handleOnLoad(){
        ...
        .then(res => {
          console.log(res);
          const [calledState, calledMode, calledBrightness, speed, calledColor, message] = res.split(',');
          state = calledState;
          if(calledState === 'ON'){
      ...
      submitButton.addEventListener('click', (e) => {
        submitConfig('/setText', `Settings=${state},${mode},${brightness},${speedControl.value},${hexToRGBA(color)},${messageInput.value.replaceAll(',', '|')}`);
      });
    </script>
  </body>
</html>
...
delay(500);
  ...
  prefs.putInt("mode", 0);
  prefs.putString("color", "255|255|255");
...
void handleLed(){
  if(prefs.getString("state") == "ON"){
    matrix.fillScreen(prefs.getDouble("brightness") * 4);
